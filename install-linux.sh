...
echo -e "${BLUE}[4/9] Updating npm to latest version...${NC}"
...
echo -e "${BLUE}[5/9] Installing build essentials...${NC}"
if command_exists apt-get; then
...
echo -e "${BLUE}[6/9] Clearing npm cache...${NC}"
npm cache clean --force

echo -e "${BLUE}[7/9] Installing global dependencies...${NC}"
npm install -g tsx nodemon drizzle-kit esbuild
if [ $? -ne 0 ]; then
    echo -e "${YELLOW}WARNING: Some global packages may not have installed${NC}"
    echo "Trying without global flag..."
fi

echo -e "${BLUE}[8/9] Removing old dependencies...${NC}"
...
echo -e "${BLUE}[9/9] Installing project dependencies...${NC}"
npm install
if [ $? -ne 0 ]; then
...
echo -e "${BLUE}[OPTIONAL] Setting up database...${NC}"
npm run db:push
if [ $? -ne 0 ]; then
